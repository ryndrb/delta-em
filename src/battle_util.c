    ...
    case ABILITYEFFECT_ON_SWITCHIN: // 0
        ...
        case ABILITY_FOREWARN:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                ...
                SET_STATCHANGER(STATID_SPA, 1, FALSE);
...
u8 ItemBattleEffects(u8 caseID, u8 battler, bool8 moveTurn)
{
    ...
    case ITEMEFFECT_NORMAL:
        if (gBattleMons[battler].hp)
        {
            ...
            LEFTOVERS:
#if B_HEAL_BLOCKING >= GEN_5
                if (gBattleMons[battler].hp < gBattleMons[battler].maxHP && !moveTurn && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK) && !IsAbilityOnOpposingSide(battler, ABILITY_UNNERVE))
#else
...
static u32 CalcDefenseStat(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 isCrit, bool32 updateFlags)
{
    ...
    else if ((gBattleMons[battlerAtk].ability == ABILITY_POWER_FIST) && gBattleMoves[move].punchingMove)
    {
        defStat = spDef;
...
u32 CalcSecondaryEffectChance(u8 battler, u8 secondaryEffectChance)
{
    if (GetBattlerAbility(battler) == ABILITY_SERENE_GRACE)
        ...
        secondaryEffectChance = gBattleMoves[gCurrentMove].secondaryEffectChance * 5;

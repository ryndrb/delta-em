        ...
        if (gBattleMons[battler].hp != 0)
        {
            ...
            case ABILITY_SOLAR_POWER:
                if (IsBattlerWeatherAffected(battler, B_WEATHER_SUN))
                {
                    ...
                    gBattleMoveDamage = GetNonDynamaxMaxHP(battler) / 32;
                    if (gBattleMoveDamage == 0)
                        gBattleMoveDamage = 1;
                    effect++;
                }
            ...
            case ABILITY_HONEY_GATHER:
				if (!BATTLER_MAX_HP(battler) && Random() % 4 == 0)
				{
        ...
        if (move != MOVE_NONE)
        {
            ...
            case ABILITY_LIGHTNING_ROD:
                if (moveType == TYPE_ELECTRIC && gMovesInfo[move].target != MOVE_TARGET_ALL_BATTLERS)
                {
                    ...
                    atk = gBattleMons[battler].attack   * gStatStageRatios[gBattleMons[battler].statStages[STAT_ATK]][0]    / gStatStageRatios[gBattleMons[battler].statStages[STAT_ATK]][1];
                    spA = gBattleMons[battler].spAttack * gStatStageRatios[gBattleMons[battler].statStages[STAT_SPATK]][0]  / gStatStageRatios[gBattleMons[battler].statStages[STAT_SPATK]][1];
...
static inline u32 CalcDefenseStat(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 isCrit, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectDef, u32 weather)
{
    ...
    else if ((gBattleMons[battlerAtk].ability == ABILITY_POWER_FIST) && gMovesInfo[move].punchingMove)
    {
        defStat = spDef;
...
u8 GetTypeEffectiveness(u8 targetId, u32 battler)
{
	...
	else
	{
		...
		{
			uq4_12_t mod3 = sTypeEffectivenessTable[gMovesInfo[moveInfo->moves[gMoveSelectionCursor[battler]]].argument][gBattleMons[targetId].type1];
			mod = uq4_12_multiply(mod, mod3);

			if (gBattleMons[targetId].type2 != gBattleMons[targetId].type1)
			{
				uq4_12_t mod4 = sTypeEffectivenessTable[gMovesInfo[moveInfo->moves[gMoveSelectionCursor[battler]]].argument][gBattleMons[targetId].type2];

...
static u32 CalcMoveBasePowerAfterModifiers(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 updateFlags)
{
    ...
    case ABILITY_STRIKER:
        if (gBattleMoves[move].kickingMove)
           uq4_12_multiply(modifier, UQ_4_12(1.2));
        break;
    ...
    case ABILITY_GROUNDED:
		if (moveType == TYPE_GROUND)
            uq4_12_multiply(modifier, UQ_4_12(1.5));
        break;
    }
    ...
    case EFFECT_SOLAR_BEAM:
        if (IsBattlerWeatherAffected(battlerAtk, (B_WEATHER_HAIL | B_WEATHER_SANDSTORM | B_WEATHER_RAIN | B_WEATHER_SNOW)) && GetBattlerAbility(gBattlerAttacker) != ABILITY_CHLOROPLAST)
            modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
        break;
...
static u32 CalcAttackStat(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 isCrit, bool32 updateFlags)
{
    ...
    case ABILITY_PSYCH:
        if (IS_MOVE_SPECIAL(move))
            uq4_12_multiply(modifier, UQ_4_12(2.0));
        break;
    case ABILITY_SLOW_START:
...
static u32 CalcFinalDmg(u32 dmg, u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, uq4_12_t typeEffectivenessModifier, bool32 isCrit, bool32 updateFlags)
{
    ...
    case ABILITY_FATAL_PRECISION:
        if (typeEffectivenessModifier >= UQ_4_12(2.0))
            uq4_12_multiply(finalModifier, UQ_4_12(1.2));
        break;
    }

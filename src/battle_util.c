...
bool32 CanAbilityAbsorbMove(u32 battlerAtk, u32 battlerDef, enum Ability abilityDef, u32 move, enum Type moveType, enum FunctionCallOption option)
{
    ...
    case ABILITY_LIGHTNING_ROD:
        if (B_REDIRECT_ABILITY_IMMUNITY >= GEN_5 && moveType == TYPE_ELECTRIC && GetBattlerMoveTargetType(battlerAtk, move) != MOVE_TARGET_ALL_BATTLERS)
        {
            effect = MOVE_ABSORBED_BY_STAT_INCREASE_ABILITY;
            u16 atk, spA;
            atk = gBattleMons[battlerDef].attack   * gStatStageRatios[gBattleMons[battlerDef].statStages[STAT_ATK]][0]    / gStatStageRatios[gBattleMons[battlerDef].statStages[STAT_ATK]][1];
            spA = gBattleMons[battlerDef].spAttack * gStatStageRatios[gBattleMons[battlerDef].statStages[STAT_SPATK]][0]  / gStatStageRatios[gBattleMons[battlerDef].statStages[STAT_SPATK]][1];
    ...
    case ABILITYEFFECT_ENDTURN:
        if (IsBattlerAlive(battler))
        {
            ...
            case ABILITY_SOLAR_POWER:
                ...
                {
                SOLAR_POWER_HP_DROP:
                    SetPassiveDamageAmount(battler, GetNonDynamaxMaxHP(battler) / 32);
            ...
            case ABILITY_HONEY_GATHER:
				if ((gBattleMons[battler].hp != gBattleMons[battler].maxHP) && Random() % 4 == 0)
				{
...
u32 IsAbilityPreventingEscape(u32 battler)
{
    if((GetBattlerAbility(battler) == ABILITY_RUN_AWAY) && 
       (IsAbilityOnOpposingSide(battler, ABILITY_ARENA_TRAP) || 
        IsAbilityOnOpposingSide(battler, ABILITY_SHADOW_TAG) || 
        IsAbilityOnOpposingSide(battler, ABILITY_MAGNET_PULL)))
        return 0;
    if(IsAbilityOnOpposingSide(battler, ABILITY_RUN_AWAY) && 
       ((GetBattlerAbility(battler) == ABILITY_ARENA_TRAP) || 
        (GetBattlerAbility(battler) == ABILITY_SHADOW_TAG) || 
        (GetBattlerAbility(battler) == ABILITY_MAGNET_PULL)))
        return 0;
...
static inline u32 CalcAttackStat(struct DamageContext *ctx)
{
    ...
    if (IsBattlerAlive(BATTLE_PARTNER(battlerAtk)))
    {
        ...
        {
        case ABILITY_FLOWER_GIFT:
            if (gBattleMons[BATTLE_PARTNER(battlerAtk)].species == SPECIES_CHERRIM_SUNSHINE && IsBattlerWeatherAffected(BATTLE_PARTNER(battlerAtk), B_WEATHER_SUN) && IsBattleMoveSpecial(move))
                modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
            break;
        default:
            break;
    ...
    case HOLD_EFFECT_LIGHT_BALL:
        if (atkBaseSpeciesId == SPECIES_PIKACHU && (B_LIGHT_BALL_ATTACK_BOOST >= GEN_4 || IsBattleMoveSpecial(move)))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        else if ((atkBaseSpeciesId == SPECIES_RAICHU || atkBaseSpeciesId == SPECIES_RAICHU_ALOLA) && (B_LIGHT_BALL_ATTACK_BOOST >= GEN_4 || IsBattleMoveSpecial(move)))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
...
static inline u32 CalcDefenseStat(struct DamageContext *ctx)
{
    ...
    else if ((ctx->abilityAtk == ABILITY_POWER_FIST) && gMovesInfo[move].punchingMove)
    {
        defStat = spDef;
...
static inline void MulByTypeEffectiveness(struct DamageContext *ctx, uq4_12_t *modifier, enum Type defType)
{
    ...
    else if (ctx->moveType == TYPE_POISON && defType == TYPE_STEEL && GetBattlerAbility(ctx->abilityAtk) == ABILITY_CORROSION && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(2.0);
        if (ctx->updateFlags)
            RecordAbilityBattle(ctx->battlerAtk, ctx->abilityAtk);
...
u32 CalcSecondaryEffectChance(u32 battler, enum Ability battlerAbility, const struct AdditionalEffect *additionalEffect)
{
    ...
    if (hasRainbow && additionalEffect->moveEffect != MOVE_EFFECT_SECRET_POWER)
        ...
        secondaryEffectChance = additionalEffect->chance * 5;

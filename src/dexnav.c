...
static void AddSearchWindowText(u16 species, u8 proximity, u8 searchLevel, bool8 hidden)
{
    ...
    if (proximity <= SNEAKING_PROXIMITY)
    {
        ...
        {
            StringCopy(gStringVar1, gAbilitiesInfo[sDexNavSearchDataPtr->moves[0]].name);
...
static u8 GetEncounterLevelFromMapData(u16 species, u8 environment)
{
    ...
    const struct WildPokemonInfo *landMonsInfo = GetProperLandMonsWithTime(headerId);
...
static void PrintCurrentSpeciesInfo(void)
{
    ...
    {
        ConvertIntToDecimalStringN(gStringVar4, 0, 0, 4);
...
bool8 TryFindHiddenPokemon(void)
{
    ...
    if ((*stepPtr) == 0 && (Random() % 100 < HIDDEN_MON_SEARCH_RATE))
    {
        ...
        case 0: //land
            ...
            {
                species = GetProperLandMonsWithTime(headerId)->wildPokemon[ChooseWildMonIndex_Land()].species;

...
{
    if (gBerryTreeWildMonHeaders[headerId].encounterTypes->landMonsInfo != NULL)
    {
        TryGenerateWildMon(gBerryTreeWildMonHeaders[headerId].encounterTypes->landMonsInfo, WILD_AREA_LAND, 0);
        BattleSetup_StartWildBattle();
        gSpecialVar_Result = TRUE;
    }
    else

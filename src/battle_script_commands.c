...
{//   Item                      1+  11+  21+  31+  41+  51+  61+  71+  81+  91+   Levels
    ...
    { ITEM_GREAT_BALL,      {  10,  10,  10,  10,   _,   _,   _,   _,   _,   _, } },
    ...
    { ITEM_ELIXIR,          {   _,   _,   _,   _,   _,   2,   3,   4,   5,   5, } },
    { ITEM_MAX_ELIXIR,      {   _,   _,   _,   _,   _,   _,   _,   2,   3,   5, } },
...
static bool32 AccuracyCalcHelper(u32 move, u32 battler)
{
    ...
    else if(GetBattlerAbility(gBattlerAttacker) == ABILITY_FATAL_PRECISION && gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_SUPER_EFFECTIVE) 
    {
        effect = TRUE;
    }
    else if (HasWeatherEffect())
    {
        if ((moveEffect == gMovesInfo[move].alwaysHitsInRain)
            && IsBattlerWeatherAffected(battler, B_WEATHER_RAIN))
            effect = TRUE;
        else if ((gBattleWeather & (B_WEATHER_HAIL | B_WEATHER_SNOW)) && moveEffect == gMovesInfo[move].alwaysHitsInHailSnow)
            effect = TRUE;
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move, u32 atkAbility, u32 defAbility, u32 atkHoldEffect, u32 defHoldEffect)
{
    ...
    case ABILITY_INNER_FOCUS:
        calc = (calc * 110) / 100; // 1.1 inner focus boost
    }
...
static void Cmd_recoverbasedonsunlight(void)
{
    ...
    if (gBattleMons[gBattlerAttacker].hp != gBattleMons[gBattlerAttacker].maxHP)
    {
        ...
        {
            if ((!(gBattleWeather & B_WEATHER_ANY) || !HasWeatherEffect() || GetBattlerHoldEffect(gBattlerAttacker, TRUE) == HOLD_EFFECT_UTILITY_UMBRELLA) && GetBattlerAbility(gBattlerAttacker) != ABILITY_CHLOROPLAST)
                gBattleStruct->moveDamage[gBattlerAttacker] = GetNonDynamaxMaxHP(gBattlerAttacker) / 2;
            else if (gBattleWeather & B_WEATHER_SUN)
                gBattleStruct->moveDamage[gBattlerAttacker] = 20 * GetNonDynamaxMaxHP(gBattlerAttacker) / 30;

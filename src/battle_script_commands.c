...
static bool32 AccuracyCalcHelper(u16 move)
{
    ...
    if(GetBattlerAbility(gBattlerAttacker) == ABILITY_FATAL_PRECISION && CalcTypeEffectivenessMultiplier(move, gBattleMoves[move].type, gBattlerAttacker, gBattlerTarget, GetBattlerAbility(gBattlerTarget), TRUE) >= UQ_4_12(2.0)) 
    {
        if (!JumpIfMoveFailed(7, move))

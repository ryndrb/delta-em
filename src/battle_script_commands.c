...
static bool32 AccuracyCalcHelper(u32 move, u32 battler)
{
    ...
    if(GetBattlerAbility(gBattlerAttacker) == ABILITY_FATAL_PRECISION && CalcTypeEffectivenessMultiplier(move, gMovesInfo[move].type, gBattlerAttacker, gBattlerTarget, GetBattlerAbility(gBattlerTarget), TRUE) >= UQ_4_12(2.0)) 
    {
...
static inline void CalculateAndSetMoveDamage(struct DamageCalculationData *damageCalcData, u32 battlerDef)
{
    ...
    if (GetMoveEffect(gCurrentMove) == EFFECT_HIDDEN_POWER)

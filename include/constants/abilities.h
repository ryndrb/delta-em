...
enum __attribute__((packed)) Ability
{
    ...
    ABILITY_PYROMANCY = ABILITIES_COUNT_GEN9, // Fire attacks are 5x more likely to inflict a burn
    ABILITY_FATAL_PRECISION = ABILITIES_COUNT_GEN9 + 1, // Super Effective Moves from this Pokemon can’t miss and receive a 20% damage boost
    ABILITY_ADRENALINE = ABILITIES_COUNT_GEN9 + 2, // Boosts Speed by 1.5x, Attack by 1.2x on first turn
    ABILITY_GROUNDED = ABILITIES_COUNT_GEN9 + 3, // Powers up ground moves (similar to Rocky Payload)
    ABILITY_POISON_ABSORB = ABILITIES_COUNT_GEN9 + 4, // Restore 1/4 HP when hit by Poison-type moves.
